...
void WifibotClient::ApprocheLogiqueNavigation(int left_speed, int right_speed)
{
	...
	/*
	/*          -----------------------------------------------------------------------------------
	/*                Nous avons défini deux seuils:  SEUIL(80) et SEUIL1(35)		      	  
	/*          -----------------------------------------------------------------------------------        
	/*          |           IRLeft				|       > 80       |       35~80       |       < 35         |          
	/*          ------------------------------------------------------------------------------------          
	/*          |                   |   > 80     |			33    	|         23       |         13         |            
	/*          |   IRRight         |  35~80     |	  	    32    	|         22       |         12         |           
	/*          |                   |   < 35	 |			31    	|         21       |         11         |                        
	/*          ------------------------------------------------------------------------------------         
	/*
	/*         Remarques:    1/  sensors_data  IR supérieur à 80:        Proche de l'Obstacle
	/*                      2/  sensors_data  IR entre 35 et 80:         L'obstacle est ni trop proche ni trop loin
	/*                      3/  sensors_data  IR inférieur à 35:         Loin de l'Obstacle
	/* ============================================================================ */

	if ((sensors_data.IRLeft >= SEUIL))
	{
		if (sensors_data.IRRight >= SEUIL)
			Env = 33;
		else if ((sensors_data.IRRight >= SEUIL1) && (sensors_data.IRRight < SEUIL))
			Env = 32;
		else
			Env = 31;
	}
	else if ((sensors_data.IRLeft >= SEUIL1) && (sensors_data.IRLeft < SEUIL))
	{
		if (sensors_data.IRRight >= SEUIL)
			Env = 23;
		else if ((sensors_data.IRRight >= SEUIL1) && (sensors_data.IRRight < SEUIL))
	...
	switch (Env)
	{
	//  Si l'on détecte que l'obstacle est plus proche du capteur gauche on tourne à droite et inversement., on tourne le robot avec moins grande vitesse;
	//  A cause des larges roues, on définit que la vitesse des roues les plus proches de l'obstacle sera moins grandes que les roues opposées 
	case 33:
	    TournerGauche(left_speed - 25, right_speed - 50);
	    Sleep(3);
		break;
	case 31:
		TournerDroite(left_speed - 50, right_speed - 25);
		Sleep(3);
		break;
	case 13:
		TournerGauche(left_speed - 25, right_speed - 50);
	...
	//  Gestion du chemin entre deux obstacles;  
	case 32:
		Avancer(left_speed, 30);
		Sleep(3);
		break;
	case 23:
		Avancer(30, right_speed);
		Sleep(3);
		break;

    //  Si le robot s'approche de l'obstacle,  on modifie les vitesses des roues du robot en fonction de la position de l'obstacle 
	case 22:
		Avancer(left_speed - 30, right_speed);
		Sleep(3);
		break;
	case 21:
		Avancer(left_speed, right_speed - 25);
		Sleep(50);
		break;
	case 12:
		Avancer(left_speed - 25,right_speed);
		Sleep(50);
		break;

    //  Si le robot est loin de l'obstacle, celui-ci avance directement; 
	case 11:
		Avancer(left_speed, right_speed);
		break;
	}
}
...
/*Fonction permettant de se connecter à la Webcam du wifibot
L'adresse IP à entrer est celle de votre wifibot*/

/*void WifibotClient::ConnectToWebcam() {
	...
	if (!vcap.open("http://192.168.1.76:8080/?action=stream?param.mjpg"))
	{
		cout << "Error opening video stream" << endl; //affichage : cout
		getchar();

	}
	cout << "Stream opened" << endl; //endl:retour à la ligne

	while (waitKey(10) < 0) //attente de 10 ms : waitKey 
	{
		vcap >> raw_image;
		imshow("Webcam", raw_image);
...
/*__________________________________________________________________________________________________________________*/
/*Plannification de trajectoire : le robot se déplace d'un point A à un point B tous les deux bien définis.         */
/*Ici, on prend en compte que le parcours est sous forme d'un couloir avec des obstacles (ou pas)                   */
/*Pour planifier la trajectoire, on utilise l'odométrie du robot (distance que pourcourt le robot)                  */
/*																													*/
/*On doit savoir la distance à laquelle se trouve le point A du point B.											*/
/*On entre cette distance dans la condition "if (sensors_data.OdometryLeft < odo+57600)",							*/
/*sachant que 57600 correspond environ à 9 mètres parcourrus. 1 mètre correspond environ à 5900						*/
/*																													*/
/*Le robot évitera les obstacles s'il y en a qui se trouvent sur son passage,										*/
/*et il s'arrêtera dès qu'il aura parcouru la distance souhaitée													*/
/*																													*/
/*L'évitement d'obstacles est basique (s'il y a un obstacle => il dévie).											*/
/*Il peut être amélioré par la fonction "ApprocheLogiqueNavigation"													*/

void WifibotClient::PlanificationDeTrajectoire(int left_speed, int right_speed)
{
	GetSensorData(&sensors_data);	//on récupère les valeurs des capteurs

									/* On affiche les valeurs des différents capteurs */
	printf("Batterie : %d\n", sensors_data.BatVoltage);
	printf("IR Right : %d\n", sensors_data.IRRight);
	printf("IR Left : %d\n", sensors_data.IRLeft);
	printf("Odometry Left : %d\n", sensors_data.OdometryLeft);
	printf("Odometry Right : %d\n", sensors_data.OdometryRight);

	printf("odo : %d\n", odo); //on affiche la variable utilisée pour gérer l'odométrie (valeur fonction de la distance parcourrue)

							   /*______CONDITION DE DISTANCE______*/
	if (sensors_data.OdometryLeft < odo + 57600) //condition de disance (57600 ~= 9mètres). Modifier la valeur 57600 à vos souhaits
	{
		Avancer(left_speed, right_speed);

		/*____________EVITEMENTS D'OBSTACLES____________*/
		if ((sensors_data.IRLeft > 10) || (sensors_data.IRRight > 10))
		{

			if ((sensors_data.IRLeft >= 70) || (sensors_data.IRRight >= 70))  //si les obstacles sont proches
			{
				if ((sensors_data.IRLeft >= 70) && (sensors_data.IRRight >= 70))  //si l'obstacle est devant le robot
				{
					TournerDroite(left_speed - 50, right_speed - 50);			  //par défaut on tourne à droite
					Sleep(3);
				}
				else
				{
					if (sensors_data.IRLeft > sensors_data.IRRight)					//si l'obstacle est à gauche du robot
					{
						TournerDroite(left_speed - 50, right_speed - 50);
						Sleep(3);
					}
					else                                                            //si l'obstacle est à droite du robot
					{
						TournerGauche(left_speed - 50, right_speed - 50);
						Sleep(3);
					}
				}

			}
			else                    //si les obstacles sont loins 
			{
				if (sensors_data.IRLeft > sensors_data.IRRight)
				{
					Avancer(left_speed, right_speed - 10);
					Sleep(50);
				}
				else
				{
					Avancer(left_speed - 10, right_speed);
					Sleep(50);
				}
			}
